...
config_dirs=(
    ...
    # "configs/GOODWebKB/university/covariate"
    # "configs/GOODTwitch/language/covariate"
...
mkdir -p "$output_dir"
result_file="$output_dir/covariate_results.txt"
...
for dir in "${config_dirs[@]}"; do
    # yaml_files=$(find "$dir" -name "*.yaml" ! -name "base.yaml" ! -name "MARIO.yaml" ! -name "GRACE.yaml" ! -name "SWAV.yaml")
    yaml_files=$(find "$dir" -name "Mixup.yaml")
    # yaml_files=$(find "$dir" -name "DANN.yaml")
    ...
    for config in $yaml_files; do
        ...
        dataset_path=$(echo "$dataset_path" | sed 's|/covariate|-covariate|')
        ...
        for ((i=1; i<=num_runs; i++)); do
            ...
            python supervised.py --config_path "$python_config_path" --ad_aug
