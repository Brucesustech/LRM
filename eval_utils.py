...
def calibrated_prediction_with_thresholds(model, data, class_thresholds=None, device=None):
    ...
    if class_thresholds is not None:
        ...
        if not isinstance(class_thresholds, torch.Tensor):
            class_thresholds = torch.tensor(class_thresholds, device=device)

        adjusted_probs = probs + class_thresholds.unsqueeze(0)
    else:
...
def calculate_optimal_thresholds(model, data, device, num_classes):

    train_mask = data.train_mask
    train_labels = data.y[train_mask].long()
    label_counts = torch.bincount(train_labels, minlength=num_classes).float()
    

    max_count = label_counts.max()

    thresholds = (max_count / (label_counts + 1e-6) - 1.0) * 0  # 0.3是调整因子
    
    thresholds[label_counts == 0] = 0.0
    
    return thresholds.to(device)


def evaluate_with_threshold_calibration(model, loader, split, criterion, config, device):

    stat = {'score': None, 'loss': None}
    if loader.get(split) is None:
        return stat
    model.eval()
    train_mask = loader['train'].dataset.data.train_mask
    train_labels = loader['train'].dataset.data.y[train_mask]
    ...
    for data in loader['train']:
        ...
        thresholds = calculate_optimal_thresholds(model, data, device, num_classes)
        break  
    ...
    for data in loader[split]:
        ...
        if split in ['val', 'test', 'id_val', 'id_test']:  
  
            probs = torch.softmax(raw_preds, dim=1)
         
            adjusted_probs = probs + thresholds.unsqueeze(0)
         
            loss = criterion(raw_preds, targets) * mask
         
            pred, target = eval_data_preprocess(data.y, adjusted_probs.log(), mask, config)
        else:
...
def evaluate_all_with_calibrated_scores(model, loader, criterion, config, device):
 
    epoch_train_stat = evaluate(model, loader, 'eval_train', criterion, config, device)  
    id_val_stat = evaluate_with_threshold_calibration(model, loader, 'id_val', criterion, config, device)
    id_test_stat = evaluate_with_threshold_calibration(model, loader, 'id_test', criterion, config, device)
    val_stat = evaluate_with_threshold_calibration(model, loader, 'val', criterion, config, device)
    test_stat = evaluate_with_threshold_calibration(model, loader, 'test', criterion, config, device)
